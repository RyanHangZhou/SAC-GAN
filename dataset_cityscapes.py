...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, img_h, img_w, class_num, layout_flag, is_train=True):
        ...
        if is_train:
            ...
            self.background_image_path = os.path.join(dataset, 'background_image'+suffix+'4')
            self.semantic_label_path = os.path.join(dataset, 'semantic_label'+suffix+'4')

        self.object_mask_list = [x.split('/')[-1] for x in glob.glob(self.object_mask_path + '/*')]
        self.img_index = random.sample(range(len(self.object_mask_list)), k=len(self.object_mask_list))
